...
export default function EditarDepartamento({ auth, departamentoId }) {
    ...
    // Cargar los datos del departamento y los propietarios al montar el componente
    useEffect(() => {
        const cargarDatos = async () => {
            try {
                ...
                // Cargar propietarios
                const responsePropietarios = await axios.get('/api/v1/admin/propietarios');
                if (responsePropietarios.data && responsePropietarios.data.data) {
                    setPropietarios(responsePropietarios.data.data);
                }

                // Cargar datos del departamento
                const responseDepartamento = await axios.get(`/api/v1/admin/departamentos/${departamentoId}`);
                if (responseDepartamento.data && responseDepartamento.data.data) {
                    const departamento = responseDepartamento.data.data;
                    ...
                    setFormData({
                        ...
                        habitaciones: departamento.habitaciones || 0,
                    ...
                    if (departamento.imagenes && departamento.imagenes.length > 0) {
                        setImagenesExistentes(departamento.imagenes);
                    }
                }
            } catch (err) {
                console.error('Error al cargar datos:', err);
                setGeneralError('No se pudieron cargar los datos del departamento. Por favor, inténtelo de nuevo.');
            } finally {
                setLoading(false);
            }
        };
    ...
    // Eliminar una imagen nueva de la lista
    const handleRemoveNewImage = (index) => {
        const newImages = [...nuevasImagenes];
        ...
        setNuevasImagenes(newImages);
        setPreviewImages(newPreviews);
    };

    // Marcar una imagen existente para eliminar
    const handleDeleteExistingImage = (id) => {
        // Añadir a la lista de imágenes a eliminar
        setImagenesToDelete([...imagenesToDelete, id]);
    };

    // Cancelar la eliminación de una imagen existente
    const handleCancelDeleteImage = (id) => {
        // Eliminar de la lista de imágenes a eliminar
        setImagenesToDelete(imagenesToDelete.filter(imageId => imageId !== id));
    };
    ...
    const validateForm = () => {
        ...
        // Validar que quede al menos una imagen después de eliminar
        const imagenesTotales = (imagenesExistentes.length - imagenesToDelete.length) + nuevasImagenes.length;
        if (imagenesTotales === 0) {
            newErrors.imagenes = 'Debe tener al menos una imagen';
        }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // Método para el envío (PUT)
            formPayload.append('_method', 'PUT');

            // Agregar las imágenes nuevas
            nuevasImagenes.forEach((imagen, index) => {
                formPayload.append(`nuevas_imagenes[${index}]`, imagen);

                // Si no hay imágenes existentes y esta es la primera, márquela como principal
                if (imagenesExistentes.length === 0 && imagenesToDelete.length === imagenesExistentes.length && index === 0) {
                    formPayload.append(`nuevas_imagenes_tipo[${index}]`, 'principal');
                } else {
                    formPayload.append(`nuevas_imagenes_tipo[${index}]`, 'galeria');
                }
            });
            ...
            if (imagenesToDelete.length > 0) {
                imagenesToDelete.forEach((id, index) => {
                    formPayload.append(`imagenes_eliminar[${index}]`, id);
                });
            }

            // Enviar los datos al servidor
            const response = await axios.post(`/api/v1/admin/departamentos/${departamentoId}`, formPayload, {
                headers: {
            ...
            if (err.response && err.response.data && err.response.data.errors) {
                ...
                // Error general
                setGeneralError('No se pudo actualizar el departamento. Por favor, inténtelo de nuevo.');
            }
    ...
    if (loading) {
        return (
            <AdminLayout auth={auth} title="Editar Departamento">
                <Head title="Editar Departamento - Inmobiliaria" />
                        ...
                        <div className="p-6">
                            ...
                            {success && (
                                <div className="mb-6 p-4 bg-green-50 border-l-4 border-green-500 text-green-700">
                                    <p>Departamento actualizado exitosamente. Redireccionando...</p>
                                </div>
                            )}
                            ...
                            <form onSubmit={handleSubmit}>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                                    ...
                                    {imagenesExistentes.length > 0 && (
                                        <div className="md:col-span-2">
                                            <h3 className="text-lg font-medium text-gray-900 mb-3">Imágenes existentes</h3>
                                                    ...
                                                    <div key={imagen.id} className="relative">
                                                        <img
                                                            ...
                                                            className={`h-24 w-full object-cover rounded-md ${
                                                                imagenesToDelete.includes(imagen.id) ? 'opacity-30' : ''
                                                            }`}
                                                        />

                                                        {imagenesToDelete.includes(imagen.id) ? (
                                                            <button
                                                                type="button"
                                                                onClick={() => handleCancelDeleteImage(imagen.id)}
                                                                className="absolute top-0 right-0 bg-blue-500 text-white rounded-full p-1 shadow-sm hover:bg-blue-600 focus:outline-none"
                                                            ...
                                                            <button
                                                                type="button"
                                                                onClick={() => handleDeleteExistingImage(imagen.id)}
                                            ...
                                            {imagenesToDelete.length > 0 && (
                                                <p className="mt-2 text-sm text-red-500">
                                                    {imagenesToDelete.length} imagen(es) marcada(s) para eliminar. Los cambios se aplicarán al guardar.
                                                </p>
                                            )}
                                        </div>
                                    )}
                                    ...
                                    {/* Nuevas imágenes */}
                                    <div className="md:col-span-2">
                                        <h3 className="text-lg font-medium text-gray-900 mb-3">Añadir nuevas imágenes</h3>
                                    </div>
