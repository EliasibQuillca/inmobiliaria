                            ...
                            <div key={cliente.id} className="bg-white overflow-hidden shadow-sm hover:shadow-md transition-shadow duration-200 rounded-lg border border-gray-200">
                                <div className="p-6">
                                    ...
                                    <div className="flex space-x-2 pt-2 border-t">
                                        <button
                                            onClick={() => {
                                                setLoadingAction(`detalle-${cliente.id}`);
                                                verDetalle(cliente);
                                            }}
                                            disabled={loadingAction === `detalle-${cliente.id}`}
                                            className="flex-1 bg-gray-100 hover:bg-gray-200 text-gray-700 px-3 py-2 rounded-lg text-sm font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center"
                                        >
                                            {loadingAction === `detalle-${cliente.id}` ? (
                                        ...
                                        <button
                                            onClick={() => {
                                                setLoadingAction(`cotizar-${cliente.id}`);
                                                crearCotizacion(cliente);
                                            }}
                                            disabled={loadingAction === `cotizar-${cliente.id}`}
                                            className="flex-1 bg-blue-600 hover:bg-blue-700 text-white px-3 py-2 rounded-lg text-sm font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center"
                                        >
                                            {loadingAction === `cotizar-${cliente.id}` ? (
                                                <svg className="animate-spin h-4 w-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                                    <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                                                    <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                                                </svg>
